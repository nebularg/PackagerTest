...
if [ -z "$skip_zipfile" ]; then
	...
	if [ -n "$upload_github" ]; then
		...
		release_id=$( curl --trace-ascii "check_release${classic:+_2}.txt" -sS "https://api.github.com/repos/$project_github_slug/releases/tags/$tag" | jq '.id // empty' )
		if [ -n "$release_id" ]; then
			echo "Updating GitHub release: https://github.com/$project_github_slug/releases/tag/$tag"
			_gh_release_url="--trace-ascii update_release${classic:+_2}.txt -X PATCH https://api.github.com/repos/$project_github_slug/releases/$release_id"
		else
			echo "Creating GitHub release: https://github.com/$project_github_slug/releases/tag/$tag"
			_gh_release_url="--trace-ascii create_release${classic:+_2}.txt https://api.github.com/repos/$project_github_slug/releases"
		fi
